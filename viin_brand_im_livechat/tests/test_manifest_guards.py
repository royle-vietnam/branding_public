...
class TestViinBrandImLivechatManifestDependsGuard(TransactionCase):
    ...
    def test_manifest_depends_on_im_livechat_and_no_brand_module_at_all(self):
        ...
        (``im_livechat``) and on NO ``viin_brand_*`` module at all - not the retired
        ``viin_brand_common``, not its ``viin_brand_web`` successor, not any future brand module."""
        depends = _load_manifest().get("depends", [])
        self.assertEqual(
            depends, ["im_livechat"],
            "viin_brand_im_livechat must depend on exactly ['im_livechat'] and nothing else: %r"
            % (depends,),
        )
        brand_edges = [entry for entry in depends if entry.startswith("viin_brand")]
        self.assertFalse(
            brand_edges,
            "viin_brand_im_livechat must depend on NO viin_brand_* module (it consumes nothing "
