...
technique): ``viin_brand_mail_plugin`` was audited by direct inspection
(``grep -rn 'viin_brand_common' viin_brand_mail_plugin/``) and consumes
...
class TestViinBrandMailPluginManifestDependsGuard(TransactionCase):
    """``viin_brand_mail_plugin``'s manifest must not depend on ``viin_brand_common``."""
    ...
    def test_manifest_does_not_depend_on_viin_brand_common(self):
        ...
        self.assertNotIn(
            ...
            "viin_brand_mail_plugin/__manifest__.py 'depends' must not declare "
